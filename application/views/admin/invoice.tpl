...
-->
				...
				{foreach $items as $i}
					<div id="remove{$i['id']}" class="modal hide fade" style="margin-top: auto">
						...
						<div class="modal-footer">
							<a class="btn" id="removeButton{$i['id']}" href="#" data-url="{url}admin/invoice/removeItem?id={$i['id']}&ic={$i['item_code']}&qty={$i['quantity']}&invoice={$temp_inv_no}" onClick="removeItem({$i['id']}); return false;">Yes</a>
		...
		<div id="main-wrapper">
			<form method="post" action="{url}admin/invoice/placeInvoice" style="margin: 0 0">
				<fieldset>
					...
					<div>
						...
						<a data-url="{url}admin/invoice/reset?no={$temp_inv_no}" class="btn" type="button" id="cancel_inv" disabled="disabled">Reset Invoice</a>
						<a href="{url}admin" class="btn" type="button" id="back">Back</a>
					</div>
				</fieldset>
			</form>
		</div>
